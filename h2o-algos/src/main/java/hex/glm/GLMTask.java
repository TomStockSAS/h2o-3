...
//          double b = scale * _beta[numOff + cid];
          if (c.isSparseZero()) {
            ...
            betaIndex = updateEtas(chks, chkIndex, etas, vals, off, _beta, betaIndex, 
                    _dinfo.isInteractionVec(chkIndex), nvals, scale);
/*            for (int i = 0; i < nvals; ++i)
              etas[ids[i]] += vals[i] * b;*/
...
/*            for (int i = 0; i < nvals; ++i)
            ...
            betaIndex = updateEtas(chks, chkIndex, etas, vals, off, _beta, betaIndex, 
                    _dinfo.isInteractionVec(chkIndex), vals.length, scale);
/*            for(int i = 0; i < vals.length; ++i)  // add contribution for all rows of a numeric column from the chunk
     ...
     * num interactions, the beta and number of chunks in chks no longer track each other.
     * @param chks
     * @param cid
     * @param etas
     * @param vals
    ...
    public int updateEtas(Chunk[] chks, int cid, double[] etas, double[] vals, double off, double[] beta, 
                          int betaIndex, boolean isInteraction, int valsLen, double scale) {
      boolean treatAsNum = false;
      int betaIncrement = 1;  // by default, increment by 1
      if (isInteraction) {
        InteractionWrappedVec.InteractionWrappedChunk cInt = (InteractionWrappedVec.InteractionWrappedChunk) chks[cid];
        if (!cInt._c1IsCat && !cInt._c2IsCat) { // num by num interaction
          treatAsNum = true;
        } else {  // dealing with enum by num or num by enum interactions
          betaIncrement = cInt.vec().domain().length;
        }
      } else {
          treatAsNum = true;
      }
      ...
      for (int i = 0; i < vals.length; ++i) {
        int betaOff = treatAsNum?0:_dinfo.getInteractionOffset(chks, cid, i);
        etas[i] += (vals[i] - off) * scale * beta[betaIndex + betaOff];
      }
    ...
    public int updateGrad(Chunk[] chks, int cid, double[] gradient, double[] etas, double[] vals, double off,
                          int gradIndex, boolean isInteraction, int valsLen, double scale) {
      ...
      if (isInteraction) {
        ...
        if (!cInt._c1IsCat && !cInt._c2IsCat) { // num by num interaction
          ...
          gradIncrement = cInt.vec().domain().length;
        }
      } else {
        treatAsNum = true;
      }
      ...
      for (int i=0; i < valsLen; i++) {
        ...
        gradient[gradIndex+gradOff] += (vals[i]-off)*scale*etas[i];
      }
    ...
    private final void computeNumericGrads(Chunk [] chks, double [] etas, double [] vals, int [] ids) {
      int numCols = _dinfo._nums;
      if (numCols > 0) {
        int gradInd = _dinfo.numStart(); // index into expanded cols
        for (int cid = 0; cid < numCols; ++cid) { // go through all numeric columns expanded with enum x num interaction
          ...
          if (c.isSparseZero()) {
            ...
            gradInd = updateGrad(chks, chunkIndex, _gradient, etas, vals, offset, gradInd,
                    _dinfo.isInteractionVec(chunkIndex), nVals, scale);
/*            for (int i = 0; i < nVals; ++i)
...
/*            for (int i = 0; i < nVals; ++i)
            ...
            c.getDoubles(vals, 0, vals.length, NA);
            gradInd = updateGrad(chks, chunkIndex, _gradient, etas, vals, off, gradInd, 
                    _dinfo.isInteractionVec(chunkIndex), vals.length, scale);
/*            double g = 0;
            for (int i = 0; i < vals.length; ++i)
